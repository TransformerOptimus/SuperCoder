...
func (e *NextJsUpdateCodeFileExecutor) UpdateReGeneratedCodeFile(response Response, step steps.UpdateCodeFileStep) error {
	...
	if strings.Contains(response.FileName, "app/") {
		filePath = config.WorkspaceWorkingDirectory() + "/stories/" + step.Project.HashID + "-node/" + step.Story.HashID + "/" + response.FileName
	} else {
		filePath = config.WorkspaceWorkingDirectory() + "/stories/" + step.Project.HashID + "-node/" + step.Story.HashID + "/app/" + response.FileName
	}
...
func (e NextJsUpdateCodeFileExecutor) UpdateCodeFile(llmResponse, fileName string, step steps.UpdateCodeFileStep) error {
	...
	if step.File != "" {
		storyDir := config.WorkspaceWorkingDirectory() + "/stories/" + step.Project.HashID + "-node/" + step.Story.HashID + "/app/" + fileName
